<template>
  <a-table
      ...
      size="middle"
      :data-source="data"
      :pagination="showPage ? pageOption : false"
    ...
    <template v-for="slotItem in columns.filter(item => item.slots)"
      ...
      <template v-else>
        ...
        <div v-if="slotItem.slots.customRender == 'action'" :key="slotItem.slots.customRender" class="actions">
          <!--        对表格的操作动作start-->
          <template v-for="(action, index) in (Array.isArray(actions) ? actions : actions(slotProps.record))">
...
export default defineComponent({
  ...
  setup(props: Props, {attrs, emit, slots}) {
    ...
    const refreshTableData = async (params = {}) => {
      ...
      state.loading = true
      const {[props.dataKey]:data, count:total} = await props.getListFunc(params).finally(() => state.loading = false)
