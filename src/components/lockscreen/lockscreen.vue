  ...
  <div @keyup="unLockLogin(true)" @mousedown.stop @contextmenu.prevent :class="{unLockLogin: isShowLogin}"
    ...
    <template v-if="!isShowLogin">
      <div class="lock-box">
        ...
        <h6 class="tips">需重新输入登录密码解锁进入系统。</h6>
      </div>
      <!--      华为充电-->
    ...
    <template v-if="isShowLogin">
      <div class="login-box">
        ...
        <div class="username">{{ loginForm.userPhone }}</div>
        <a-input-search
            v-model:value="loginForm.userPsw"
            type="password"
            autofocus
            placeholder="请输入登录密码"
            size="large"
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    const state = reactive({
      ...
      loginForm: {
        userPhone: store.getters.userInfo.userPhone || store.getters.userInfo.userName,
        userPsw: '',
      }
    })
    ...
    // 登录
    const onLogin = async () => {
      if (state.loginForm.userPsw.trim() == '') return message.warn('请填写密码')
      const params = {...state.loginForm}
      state.loginLoading = true
      // params.userPsw = md5(params.userPsw)
      const {manageUser, status, msg} = await store.dispatch(UserActionTypes.Login, params)
      state.loginLoading = false
      message.destroy()
      
      if (status == 0 && manageUser) {
        Modal.destroyAll()
